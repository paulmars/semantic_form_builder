module SemanticFormHelper
  ...
  def semantic_form_right_column_size(size, unit = "em")
    <<-FORMSIZE
    <style type="text/css">
      form.semantic_form div.textarea-field div.input{ width:#{size}#{unit}; }
      form.semantic_form div.textarea-field div.input textarea{ width:#{size}#{unit}; }
    </style>
    FORMSIZE
  end
  ...
  def remote_semantic_form_for(object_name, *args, &proc)
    args[0] ||= {}
    args[0][:builder] ||= SemanticFormBuilder
    args[0][:html]    ||= {}
    args[0][:html][:class] ||= ""
